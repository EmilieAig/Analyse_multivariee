...
# Extraire PRESS
press_values <- pls_model$validation$PRESS[1, ]
...
text(6, max(press_values)*0.9, "6 comp.", col = "red", pos = 4)
grid()
...
plot(1:length(r2_values), r2_values,
     ...
     main = "Variance de Y expliquée",
     col = "darkgreen", pch = 19)
abline(v = 6, col = "red", lty = 2, lwd = 2)
abline(h = r2_values[6], col = "red", lty = 2, lwd = 1)
text(6, r2_values[6], paste0("R²=", round(r2_values[6], 3)), pos = 4, col = "red")
grid()
...
optimal_ncomp_choisi <- 6  # Choix raisonné

cat("\n=== CHOIX RAISONNÉ ===\n")
cat("Composante minimisant PRESS :", which.min(press_values), "(sur-ajustement)\n")
cat("Composante choisie (compromis) :", optimal_ncomp_choisi, "\n\n")

cat("Comparaison :\n")
cat("- 6 composantes  : R² =", round(r2_values[6], 3), 
    "  PRESS =", round(press_values[6], 1), "\n")
cat("- 20 composantes : R² =", round(r2_values[20], 3), 
    "  PRESS =", round(press_values[20], 1), "\n")
cat("Gain avec 20 comp. :", round((r2_values[20] - r2_values[6])*100, 2), 
    "% (ne justifie pas la complexité)\n\n")

# Ajuster le modèle final avec 6 composantes
pls_final <- plsr(Y ~ X, 
                  ncomp = optimal_ncomp_choisi,
                  scale = TRUE)

# Calculer le R² du modèle final
Y_pred_pls <- predict(pls_final, ncomp = optimal_ncomp_choisi)
r2_pls <- cor(Y, Y_pred_pls)^2
mse_pls <- mean((Y - Y_pred_pls)^2)

cat("=== MODÈLE PLS FINAL ===\n")
cat("Nombre de composantes :", optimal_ncomp_choisi, "\n")
cat("R² sur données d'entraînement :", round(r2_pls, 4), "\n")
cat("MSE :", round(mse_pls, 4), "\n\n")

# Graphique
plot(Y, Y_pred_pls,
     xlab = "log(Densité + 1) observé",
     ylab = "log(Densité + 1) prédit",
     main = paste0("Modèle PLS (", optimal_ncomp_choisi, " comp.)\nR² = ", 
                   round(r2_pls, 3)),
     pch = 20,
...
# Récupérer les loadings (contributions des variables aux composantes)
loadings_pls <- loadings(pls_final)

# Récupérer les scores (coordonnées des individus sur les composantes)
scores_pls <- scores(pls_final)

# --- 1) ANALYSER LES LOADINGS (quelles variables contribuent le plus) ---
cat("\n=== TOP 10 DES VARIABLES PAR COMPOSANTE ===\n\n")
...
for (comp in 1:n_comp) {
  ...
  load_sorted <- sort(abs(load_comp), decreasing = TRUE)
  
  # Afficher le top 10
  print(head(load_sorted, 10))
  cat("\n")
}

# --- 2) CERCLES DE CORRÉLATION (version simple) ---
cat("\n=== CERCLES DE CORRÉLATION ===\n")

# Calculer les corrélations entre variables originelles et composantes
X_matrix <- as.matrix(var_explicatives)
correlations <- cor(X_matrix, scores_pls)

# Fonction simple pour tracer un cercle de corrélation
plot_cercle <- function(correlations, comp1, comp2) {
  
  # Graphique vide
  plot(0, 0, xlim = c(-1, 1), ylim = c(-1, 1),
       xlab = paste("Composante", comp1),
       ylab = paste("Composante", comp2),
       main = paste("Cercle de corrélation - Plan", comp1, "-", comp2),
       type = "n", asp = 1)
  ...
  for (i in 1:nrow(correlations)) {
    ...
    # Ajouter le nom si bien représenté
    if (contrib[i] > 0.5) {
      text(x * 1.15, y * 1.15, 
...
# Tracer les 3 plans principaux
par(mfrow = c(2, 2))
plot_cercle(correlations, 1, 2)
plot_cercle(correlations, 1, 3)
plot_cercle(correlations, 2, 3)
par(mfrow = c(1, 1))

# --- 3) NUAGE DES INDIVIDUS ---
cat("\n=== NUAGE DES INDIVIDUS ===\n")

# Créer un gradient de couleurs selon Y
Y_norm <- (treedensity_log - min(treedensity_log)) / 
          (max(treedensity_log) - min(treedensity_log))
couleurs <- rgb(Y_norm, 0, 1 - Y_norm, 0.6)
...
plot(scores_pls[, 1], scores_pls[, 2],
     ...
     pch = 20, col = couleurs, cex = 1)
abline(h = 0, v = 0, lty = 2, col = "gray50")

legend("topright",
       legend = c("Densité élevée", "Densité faible"),
       col = c("red", "blue"),
       pch = 20,
       cex = 0.8)
...
# Variance de X expliquée
var_X <- pls_final$Xvar / sum(pls_final$Xvar) * 100
cat("\nVariance de X expliquée par composante :\n")
for (i in 1:n_comp) {
  cat(sprintf("Comp %d : %.2f%%\n", i, var_X[i]))
}

# Variance de Y expliquée - R² cumulé
cat("\nR² cumulé de Y par nombre de composantes :\n")
r2_result <- R2(pls_final, estimate = "train")
r2_cumul <- as.vector(r2_result$val)  # Convertir en vecteur simple

for (i in 1:n_comp) {
  cat(sprintf("%d comp : R² = %.4f (%.2f%%)\n", i, r2_cumul[i], r2_cumul[i] * 100))
}

# Variance de Y apportée par chaque composante (incrément)
cat("\nVariance de Y apportée par chaque composante (incrément) :\n")
var_Y_increment <- c(r2_cumul[1], diff(r2_cumul[1:n_comp]))
for (i in 1:n_comp) {
  cat(sprintf("Comp %d : +%.4f (+%.2f%%)\n", i, var_Y_increment[i], var_Y_increment[i] * 100))
}
